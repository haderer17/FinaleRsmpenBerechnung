document.addEventListener("DOMContentLoaded", () => {
  ...
  function drawRamp(R,h,thetaDeg){
    ...
    const r = R*scale;
    ctx.beginPath();
    ctx.arc(cx,cy,r,Math.PI,Math.PI+theta,false);
    ctx.lineWidth = 4;
  ...
  function calculate(){
    ...
    const given = [h,theta,R].filter(v=>!isNaN(v)).length;
    if(given<2){alert("Bitte mindestens zwei Werte eingeben!");return;}

    if(!R) R = h/(1-Math.cos(deg2rad(theta)));
    else if(!theta){
      const ratio = 1-h/R;
      if(ratio>1||ratio<-1){alert("Ungültige Werte: h darf nicht größer als 2·R sein!"); return;}
      theta = rad2deg(Math.acos(ratio));
    } else if(!h) h = R*(1-Math.cos(deg2rad(theta)));

    const x = R*Math.sin(deg2rad(theta));
    const arc = R*deg2rad(theta);
    ...
    outX.textContent = `Horizontale Länge = ${round(x)} m`;
  ...
  resetBtn.addEventListener("click",()=>{
    ...
    outX.textContent="Horizontale Länge = — m";
